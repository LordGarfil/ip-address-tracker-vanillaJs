    ...
    $.ajax('http://open.mapquestapi.com/geocoding/v1/reverse?', {
        ...
        success: function (res) {
          ...
          const formattedRes = res.results[0].locations[0]
            ...
            const location = `This is <strong>${formattedRes.adminArea1}</strong>`
          
          movePositionMark(mymap, formattedRes.displayLatLng.lat, formattedRes.displayLatLng.lng, location);
         mapOnClick(mymap, location)
        },
        error: function (err) {
          alert(err);
